...
对话框是Windows 编程的必备功能之一。 如果我们正在创建一个完整的应用，将会有一个阶段需要以某种形式的对话框。 对话框可以是编辑框、单选按钮、复选框等形式。 对话框有两种形式：模式对话框和非模式对话框。 模式对话框需要立即响应，而非模式对话框更像是浮动框，不需要立即响应。
