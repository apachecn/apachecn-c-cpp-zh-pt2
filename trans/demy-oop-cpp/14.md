...
接下来，我们利用重载`operator[]`使用`studentBody[0] = s1;`在我们的`deque`中插入`Student`。请注意，`operator[]`不对我们的`deque`做任何边界检查!在此语句中，我们将`Student``s1`插入到`deque`中的![](image/B15702_14_002.png)位置，而不是曾经占据该位置的`Student`。一个更安全的选择是使用`deque::at()`方法，该方法将包含边界检查。关于上述分配，我们还希望确保`operator=`对于`Person`和`Student`都已被重载，因为每个类都动态分配了数据成员。
