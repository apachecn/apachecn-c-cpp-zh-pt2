...
我们将建模的应用将涉及课程注册系统和等待列表的概念。正如我们之前在*Q第 2*的[*第 10 章*](10.html#_idTextAnchor386)，*实现关联，聚合和组合*中看到的，我们将对与许多`Course`实例关联的`Student`和与许多`Student`实例关联的`Course`进行建模。当我们模拟等待名单时，观察者模式将发挥作用。
