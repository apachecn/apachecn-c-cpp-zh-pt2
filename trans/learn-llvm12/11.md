...
LLVM附带几个**消毒剂**。 这些是向该仪器传递**中间表示**(**IR**)，以检查应用的某些错误行为。 通常，它们需要库支持，这是`compiler-rt`项目的一部分。 可以在 Clang 中启用消毒剂，这使得它们使用起来非常舒适。 在以下部分中，我们将了解可用的消毒器，即`address`、`memory`和`thread`。 我们先来看看`address`消毒剂。
