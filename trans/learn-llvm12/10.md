...
8.  在构造函数的主体中，我们添加生成器来搜索当前进程中的符号。 `GetForCurrentProcess()`方法是特殊的，因为返回值包装在`Expected<>`模板中，这表明也可以返回`Error`对象。 但我们知道不会发生错误-当前进程最终会运行！ 因此，我们使用`cantFail()`函数解开结果，如果发生错误，该函数将终止应用程序：
