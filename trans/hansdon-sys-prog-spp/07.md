...
`mmap()`函数可用于将来自不同来源的内存映射到程序中。例如，如果要将设备内存放入应用中，则可以使用`mmap()`。如果将`MAP_ANONYMOUS`传递给`mmap()`，则它可以用于分配内存，与使用`malloc()`和`free()`分配内存的方式相同。在前面的示例中，`mmap()`用于分配标记为读/写的 4k 页内存。`MAP_PRIVATE`的使用告诉`mmap()`您不打算与其他应用共享此内存 (例如，用于进程间通信)。这种方式映射内存与`malloc()`/`free()`相比有一些优点和缺点。
