...
我们也可以使用 Boost 绑定表达式和 Phoenix 角色来生成动作。 因此，我们可以编写`+qi::uint_[boost::bind(print, ::_1)]`来对每个解析的数字调用`print`。 占位符`::_1`到`::_9`由全局名称空间中的 Boost 绑定库定义。 勇气号提供了可用于各种动作的凤凰演员。 下面的代码片段显示了将解析的数字添加到向量中的方法：
