...
Windows的应用也被称为基于 GUI (图形用户界面) 的应用。用户与 app 的交互是通过鼠标和键盘完成的。记事本应用是与 Windows 操作系统捆绑在一起的基于 GUI 的应用的示例。以下屏幕截图显示了记事本应用:
...
但是，为什么我们要获得 GUI 应用的控制台？答案是，默认Code::Blocks创建调试目标作为控制台应用。我们可以通过导航到**项目** | **属性…** 菜单选项，然后单击**构建目标**选项卡来确认这一点。请参阅以下屏幕截图:
...
*   **Qt** (读作 “可爱”): It是由**Digia**开发的开源、跨平台的 GUI 工具包。Qt 根据商业许可和 GPL/LGPL 许可获得许可。它可在各种平台上使用，包括 Windows，Linux，Mac 等。Qt 绘制 GUI 是一个自定义绘制的 UI，可能与平台上的标准应用不同。
...
为了在 Linux 平台上编译，我们将使用`Makefile`。请记住，我们还可以使用Code::Blocks wxWidgets 项目向导来生成针对 Linux 平台的项目。但是，我认为开发人员应该熟悉`Make`工具。
...
我们的应用现在工作!我们在 code::Blocks 的帮助下编写了大部分与 GUI 相关的代码。只有手动编写的代码是我们插入以打开文本文件的代码。Code::Blocks 提供了一个优秀的跨平台和快速的应用开发平台。我们可以使用这种方法轻松开发带有Code::Blocks的 GUI 应用。
...
在本章中，我们学习了使用 Win32 api 和Code::Blocks的 Windows 应用开发。然后，我们专注于 GUI 工具包，并使用 wxWidgets 工具包开发了第一个适用于 Windows 和 Linux 的应用。
