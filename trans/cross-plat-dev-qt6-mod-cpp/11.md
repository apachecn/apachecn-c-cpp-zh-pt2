...
要翻译 Qt Quick应用，请使用`qsTr()`方法标记`.qml`文件内的字符串。 您可以为单个 QML 文件创建翻译文件，如下所示：
...
15.  最后一步是运行qmake 并构建应用。 然后，点击左下角的**Run**按钮。
...
在上一节中，我们讨论了Qt 小部件中的国际化。 在本节中，我们将讨论国际化 Qt Quick 应用的不同方面。 Qt Quick 应用中的底层本地化方案类似于 Qt 窗口小部件应用。 Qt Quick 中也使用了 Qt 语言手册中描述的相同工具集。 您可以翻译同时使用 C++ 和 QML 的应用。
