...
您可以使用静态链接或动态链接构建Qt 应用。 当您构建应用时，链接器使用这两种方法中的任何一种将所有使用的库函数复制到可执行文件中。 我们假设您已经知道这两种方法。 在本节中，我们将讨论何时对 Qt 应用使用静态链接和何时使用动态链接。
...
关闭**系统属性**屏幕并启动Qt 命令提示符。 然后，您可以使用以下语法为基于 Qt 小部件的应用创建部署包：
...
8.  打开清单文件。 您将看到您的Android 应用的几个选项。
