...
前几章讨论了构建 Windows 应用商店应用的各种细节：从基本的 Windows 运行时概念，到构建用户界面，再到使用独特的 Windows 8 功能(例如动态磁贴、合同)。 剩下的就是构建你的应用，最后提交到应用商店，这样每个人都可以享受它。
...
我们必须选择版本配置来创建和测试包。 测试 Debug 配置无论如何都会失败。 上传到应用商店的内容必须仅为发布版本，否则应用将无法通过认证。 我们可以选择要为其创建包的所需体系结构。 建议使用 x86和 ARM-ARM是 Windows RT 计算机上唯一可用的体系结构，因此应受支持。 X86 是基于英特尔/AMD 架构的不错选择。
...
运行WACK 是在商店中完成的第一步；这意味着如果应用没有通过本地 WACK 测试，它肯定也不会通过商店的认证。 选择后，将出现以下对话框：
