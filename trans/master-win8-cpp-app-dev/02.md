...
WinRT 还定义了属性的概念，这些属性实际上是变相的方法。 属性可以有 getter 和/或 setter。 由于 C++ 没有属性的概念，因此这些属性被建模为以`get_`或`put_`开头的方法，而 C++/CX 为方便起见提供了对属性的类似字段的访问。
...
使用禁止的 API 无法编译；这是因为 Windows API 标头已更改为基于两个常量`WINAPI_PARTITION_APP`(适用于 Store 应用)和`WINAPI_PARTITION_DESKTOP`(适用于桌面应用)有条件地编译。
