...
对`send_to`和`receive_from`的调用是同步的，仅当数据完全传递到操作系统(`send_to`)或完全被应用接收(`receive_from`)时才返回。 如果多个客户端实例同时向服务器发送消息，服务器一次仍只能处理一条消息，因此客户端会排队等待响应。 当然，如果客户端没有等待响应，它们可能都已发送消息并退出，但服务器仍会连续接收这些消息。
